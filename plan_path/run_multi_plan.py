...
from run_joint_plan import BFSPathPlanner
...
class SequencePlanner:
    ...
    def __init__(self, asset_folder, assembly_dir):
        ...
        for part_id in assembly.keys():
            self.graph.add_node(part_id)

        self.num_parts = len(assembly)
    ...
    def plan_path(self, move_id, still_ids, rotation, sdf_dx, collision_th, min_sep, force_mag, frame_skip,
        max_time, seed, render, record_path, save_dir, n_save_state, return_contact=False):

        path_planner = BFSPathPlanner(
            self.asset_folder, self.assembly_dir,
            move_id, still_ids,
            rotation, sdf_dx, collision_th, min_sep, force_mag, frame_skip, save_sdf=True
        )
...
class ProgressiveQueueSequencePlanner(SequencePlanner):
    ...
    def plan_sequence(self, rotation, sdf_dx, collision_th, min_sep, force_mag, frame_skip,
        ...
        while True:
            ...
            status, t_plan = self.plan_path(move_id, still_ids,
                False, sdf_dx, collision_th, min_sep, force_mag, frame_skip,
            ...
            if status in self.failure_status and rotation:
                status, t_plan_rot = self.plan_path(move_id, still_ids,
                    True, sdf_dx, collision_th, min_sep, force_mag, frame_skip,
                    path_max_time, max_depth, curr_seed, render, record_path, curr_save_dir, n_save_state)
                t_plan += t_plan_rot
...
if __name__ == '__main__':
    ...
    seq_status, sequence, seq_count, t_plan = seq_planner.plan_sequence(
        args.rotation, args.sdf_dx, args.collision_th, args.min_sep, args.force_mag, args.frame_skip,
