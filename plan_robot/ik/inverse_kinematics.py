...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None, max_iter=None, orientation_mode=None, no_position=False, optimizer="L-BFGS-B"):
    """
    ...
    with np.errstate(divide='ignore', invalid='ignore', over='ignore'): # ignore numerical warnings
        ...
        if optimizer == "L-BFGS-B":
            res = scipy.optimize.minimize(optimize_scalar, chain.active_from_full(starting_nodes_angles), bounds=real_bounds, method='L-BFGS-B')
        elif optimizer == 'SLSQP':
            res = scipy.optimize.minimize(optimize_scalar, chain.active_from_full(starting_nodes_angles), bounds=real_bounds, method='SLSQP')
            ...
            for factor in range(len(real_bounds)):
                ...
                cons.append(l)
                cons.append(u)
            res = scipy.optimize.minimize(optimize_scalar, chain.active_from_full(starting_nodes_angles), constraints=cons, method='COBYLA', tol=1e-5)
    ...
    # print('IK result:', res.success, res.status, res.message)
