...
Eigen::Vector3f Viewer::GetLookAt() const {
  Eigen::Matrix4f camera = view_matrix_.inverse();
  Eigen::Vector3f newEye = camera.block<3, 1>(0, 3);
  Eigen::Vector3f newCenter = newEye - camera.block<3, 1>(0, 2);
  return newCenter;
}

Eigen::Vector3f Viewer::GetPosition() const {
  return -view_matrix_.topLeftCorner(3, 3).transpose() *
    view_matrix_.topRightCorner(3, 1);
}
