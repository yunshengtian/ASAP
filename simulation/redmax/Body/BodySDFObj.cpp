...
void BodySDFObj::load_visual_mesh(std::string visual_filename, Vector3 scale) {
    std::vector<tinyobj::shape_t> obj_shape;
    std::vector<tinyobj::material_t> obj_material;
    tinyobj::attrib_t attrib;
    std::string err;
    tinyobj::LoadObj(&attrib, &obj_shape, &obj_material, &err, visual_filename.c_str());

    int num_vertices = (int)attrib.vertices.size() / 3;
    _Vr.resize(3, num_vertices);
    for (int i = 0;i < num_vertices;i++) {
        _Vr.col(i) = Vector3(attrib.vertices[i * 3] * scale[0], 
            attrib.vertices[i * 3 + 1] * scale[1],
            attrib.vertices[i * 3 + 2] * scale[2]);
    }
    
    int num_elements = (int)obj_shape[0].mesh.indices.size() / 3;
    _Fr.resize(3, num_elements);
    for (int i = 0;i < num_elements;i++) {
        _Fr.col(i) = Vector3i(obj_shape[0].mesh.indices[i * 3].vertex_index,
            obj_shape[0].mesh.indices[i * 3 + 1].vertex_index,
            obj_shape[0].mesh.indices[i * 3 + 2].vertex_index);
    }
}

void BodySDFObj::get_rendering_objects(
    ...
    if (_rendering_mesh_exists) {
        vertex = _Vr.cast<float>();
    } else {
        vertex = _V.cast<float>();
    }
    ...
    object_option.SetVectorOption("specular", 0.2f, 0.2f, 0.2f);
    object_option.SetFloatOption("shininess", 0.01f);
    // object_option.SetFloatOption("shininess", 70.8f);

    _animator = new BodyAnimator(this);

    vertex_list.push_back(vertex);
    if (_rendering_mesh_exists) {
        face_list.push_back(_Fr);
    } else {
