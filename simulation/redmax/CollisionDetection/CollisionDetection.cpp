...
// detect the collision between a general body and a SDF body
void collision_detection_general_MultiSDF(const Body* contact_body, const BodyMultiSDFObj* SDF_body, std::vector<Contact>& contacts) {
    BodyMultiSDFObj* SDF_body_ = const_cast<BodyMultiSDFObj*>(SDF_body);

    // AABB bounding box check
    if (dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(contact_body)) != nullptr) {
        BodyMultiSDFObj* contact_sdf_body_ = dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(contact_body));
        std::vector<std::pair<Vector3, Vector3>> aabb1s = contact_sdf_body_->get_AABBs();
        std::pair<Vector3, Vector3> aabb2 = SDF_body_->get_AABB();
        for (int i = 0; i < aabb1s.size(); ++i) {
            std::pair<Vector3, Vector3> aabb1 = aabb1s[i];
            for (int axis = 0; axis < 3;axis ++) {
                if (aabb1.second[axis] < aabb2.first[axis] - 1e-6 || aabb2.second[axis] < aabb1.first[axis] - 1e-6) {
                    return;
                }
    ...
    if (contacts.size() > 0) {
        const_cast<Body*>(contact_body)->add_contact_body(SDF_body->_name);
        const_cast<BodyMultiSDFObj*>(SDF_body)->add_contact_body(contact_body->_name);
    }
}
