...
void ForceGeneralMultiSDFContact::set_stiffness(dtype kn, dtype kt) {
    _kn = kn;
    _kt = kt;
}

void ForceGeneralMultiSDFContact::set_friction(dtype mu) {
    _mu = mu;
}

void ForceGeneralMultiSDFContact::set_damping(dtype damping) {
    _damping = damping;
}

void ForceGeneralMultiSDFContact::set_scale(dtype scale) {
    _scale = scale;
}

void ForceGeneralMultiSDFContact::computeForce(VectorX& fm, VectorX& fr, bool verbose) {
    auto t0 = clock();
    ...
    collision_detection_general_MultiSDF(_contact_body, _SDF_body, contacts);
...
void ForceGeneralMultiSDFContact::computeForceWithDerivative(
...
void ForceGeneralMultiSDFContact::computeForce(std::vector<Contact> &contacts, VectorX& fm, bool verbose) {
...
void ForceGeneralMultiSDFContact::test_derivatives() {
    std::cerr << "**************************** General-MultiSDF Collision Derivatives ***************************" << std::endl;
    // srand(time(0));
    srand(1000);
...
void ForceGeneralMultiSDFContact::test_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
    collision_detection_general_MultiSDF(_contact_body, _SDF_body, contacts);
    ...
    print_error("General-MultiSDF Collision Derivatives: Km(q1) ", Km.leftCols(6), Km_fd.leftCols(6));
    ...
    this->_SDF_body->_E_0i = E2;
    print_error("General-MultiSDF Collision Derivatives: Km(q2) ", Km.rightCols(6), Km_fd.rightCols(6));
    ...
    this->_contact_body->_phi = phi1;
    print_error("General-MultiSDF Collision Derivatives: Dm(phi1) ", Dm.leftCols(6), Dm_fd.leftCols(6));
    ...
    this->_SDF_body->_phi = phi2;
    print_error("General-MultiSDF Collision Derivatives: Dm(phi2) ", Dm.rightCols(6), Dm_fd.rightCols(6));

    test_design_derivatives_runtime();
}

void ForceGeneralMultiSDFContact::test_design_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
    ...
    if (contacts.size() > 0) {
        ...
        for (int ii = 0;ii < 1;ii++) {
            ...
            print_error("General-MultiSDF Collision Derivatives: dfm_dp1", dfm_dp.leftCols(_sim->_ndof_p1), dfm_dp_fd.leftCols(_sim->_ndof_p1));
            print_error("General-MultiSDF Collision Derivatives: dfm_dp2", dfm_dp.middleCols(_sim->_ndof_p1, _sim->_ndof_p2), dfm_dp_fd.middleCols(_sim->_ndof_p1, _sim->_ndof_p2));
            print_error("General-MultiSDF Collision Derivatives: dfm_dp3", dfm_dp.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2, _sim->_ndof_p3), dfm_dp_fd.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2, _sim->_ndof_p3));
            print_error("General-MultiSDF Collision Derivatives: dfm_dp6", dfm_dp.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2 + _sim->_ndof_p3 + _sim->_ndof_p4 + _sim->_ndof_p5, _sim->_ndof_p6), dfm_dp_fd.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2 + _sim->_ndof_p3 + _sim->_ndof_p4 + _sim->_ndof_p5, _sim->_ndof_p6));
            eps /= 10.;
        }
