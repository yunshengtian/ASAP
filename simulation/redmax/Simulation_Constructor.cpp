...
#include "Force/ForceGeneralMultiSDFContact.h"
...
std::string concatenate_paths(const std::string& path1, const std::string& path2) {
    bool has_separator = !path1.empty() && path1.back() == path_separator;
    std::string full_path = path1 + (has_separator ? "" : std::string(1, path_separator)) + path2;
    return full_path;
}
...
std::string find_path(std::string path, std::string asset_folder) {
    std::string new_path;
    if (!file_exists(path)) { // try relative path
        new_path = concatenate_paths(asset_folder, path);
        if (!file_exists(new_path)) // error
...
std::vector<Vector3> Simulation::parse_contact_points(std::string str) {
    ...
    contacts.clear();
    std::string filename = find_path(str, _asset_folder);
    ...
    if ((std::string)(node.name()) == "redmax") {
        ...
        for (auto contact : node.children()) 
            if ((std::string)(contact.name()) == "contact") {
                for (auto child : contact.children()) {
                    ...
                    else if ((std::string)(child.name()) == "general_MultiSDF_contact") {
                        auto it1 = _body_map.find(child.attribute("general_body").value());
                        if (it1 == _body_map.end()) {
                            std::string error_msg = "General contact body name error: " + (std::string)(child.attribute("general_body").value());
                            throw_error(error_msg);
                        }
                        auto it2 = _body_map.find(child.attribute("MultiSDF_body").value());
                        if (it2 == _body_map.end()) {
                            std::string error_msg = "MultiSDF contact body name error: " + (std::string)(child.attribute("MultiSDF_body").value());
                            throw_error(error_msg);
                        }
                        ForceGeneralMultiSDFContact* force = new ForceGeneralMultiSDFContact(this, it1->second, it2->second, kn, kt, mu, damping);
                        robot->add_force(force);
        ...
        if (node.child("joint")) {
            ...
            if (joint_node.attribute("stiffness")) {
                ...
                VectorX q_rest = VectorX::Zero(joint->_ndof);
                joint->set_stiffness(stiffness, q_rest);
        ...
        if (node.child("body")) {
            ...
            if (type != "MultiSDF") {
                pos = str_to_eigen(body_node.attribute("pos").value());
                Vector4 quat = str_to_eigen(body_node.attribute("quat").value());
                R = math::quat2mat(quat);
            } else {
                std::string pos_str = body_node.attribute("pos").value();
                std::string delim = ";";
                auto start = 0U;
                auto end = pos_str.find(delim);
                while (end != std::string::npos)
                {
                    Vector3 pos_i = str_to_eigen(pos_str.substr(start, end - start));
                    start = end + delim.length();
                    end = pos_str.find(delim, start);
                    poss.push_back(pos_i);
                }
                Vector3 pos_i = str_to_eigen(pos_str.substr(start, end));
                poss.push_back(pos_i);

                std::string quat_str = body_node.attribute("quat").value();
                start = 0U;
                end = quat_str.find(delim);
                while (end != std::string::npos)
                {
                    Vector4 quat_i = str_to_eigen(quat_str.substr(start, end - start));
                    Matrix3 R_i = math::quat2mat(quat_i);
                    start = end + delim.length();
                    end = quat_str.find(delim, start);
                    Rs.push_back(R_i);
                }
                Vector4 quat_i = str_to_eigen(quat_str.substr(start, end));
                Matrix3 R_i = math::quat2mat(quat_i);
                Rs.push_back(R_i);
            }
            dtype density = 1.0;
            ...
            if (type == "cuboid") {
            ...
            } else if (type == "mesh" || type == "SDF" || type == "MultiSDF" || type == "BVH") {
                BodyMeshObj::TransformType transform_type;
                BodyMultiMeshObj::TransformType transform_type_multi;
                if (type != "MultiSDF") {
                    ...
                    if (body_node.attribute("transform_type")) {
                        ...
                        if (transform_type_str == "OBJ_TO_JOINT")
                            ...
                            transform_type = BodyMeshObj::TransformType::BODY_TO_JOINT;
                        else if (transform_type_str == "JOINT")
                            transform_type = BodyMeshObj::TransformType::JOINT;
                        else {
                    ...
                    transform_type_multi = BodyMultiMeshObj::TransformType::BODY_TO_JOINT;
                    if (body_node.attribute("transform_type")) {
                        std::string transform_type_str = body_node.attribute("transform_type").value();
                        if (transform_type_str == "OBJ_TO_JOINT")
                            transform_type_multi = BodyMultiMeshObj::TransformType::OBJ_TO_JOINT;
                        else if (transform_type_str == "OBJ_TO_WORLD")
                            transform_type_multi = BodyMultiMeshObj::TransformType::OBJ_TO_WOLRD;
                        else if (transform_type_str == "BODY_TO_JOINT")
                            transform_type_multi = BodyMultiMeshObj::TransformType::BODY_TO_JOINT;
                        else if (transform_type_str == "JOINT")
                            transform_type_multi = BodyMultiMeshObj::TransformType::JOINT;
                        else {
                            std::string error_msg = "Transform type error: " + transform_type_str;
                            throw_error(error_msg);
                        }
                ...
                if (type != "MultiSDF") {
                    filename = find_path(filename, _asset_folder);
                } else {
                    std::string delim = ";";
                    auto start = 0U;
                    ...
                    while (end != std::string::npos)
                    {
                        std::string filename_i = filename.substr(start, end - start);
                        filename_i = find_path(filename_i, _asset_folder);
                        start = end + delim.length();
                        end = filename.find(delim, start);
                        filenames.push_back(filename_i);
                    }
                    std::string filename_i = filename.substr(start, end);
                    filename_i = find_path(filename_i, _asset_folder);
                    filenames.push_back(filename_i);
                }
                if (type == "mesh") {
                    body = new BodyMeshObj(this, joint, filename, R, pos, transform_type, density, scale, adaptive_sample);
                } else if (type == "SDF" || type == "MultiSDF" || type == "BVH") {
                    dtype dx = 0.05;
                    ...
                    if (type == "SDF") {
                        ...
                        if (body_node.attribute("visual_filename")) {
                            visual_filename = body_node.attribute("visual_filename").value();
                            visual_filename = find_path(visual_filename, _asset_folder);
                        } else {
                            visual_filename = "";
                        }
                        body = new BodySDFObj(this, joint, filename, visual_filename, R, pos, dx, res, col_th, transform_type, density, scale, adaptive_sample, load_sdf, save_sdf);
                    } else if (type == "MultiSDF") {
                        body = new BodyMultiSDFObj(this, joint, filenames, Rs, poss, dx, res, col_th, transform_type_multi, density, scale, adaptive_sample, load_sdf, save_sdf);
                    } else if (type == "BVH") {
                        ...
                        if (body_node.attribute("BVH_mesh_filename")) {
                            ...
                            filename_BVH_mesh = find_path(filename_BVH_mesh, _asset_folder);
                        }
                ...
                if (body_node.attribute("mesh")) {
                    ...
                    visual_mesh_filename = find_path(visual_mesh_filename, _asset_folder);
            ...
            if (body_node.attribute("texture")) {
                ...
                std::string rgba_str = body_node.attribute("rgba").value();
                if (dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(body)) != nullptr) {
                    BodyMultiSDFObj* body_ = dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(body));
                    std::vector<Vector3> rgbs;
                    std::string delim = ";";
                    auto start = 0U;
                    auto end = rgba_str.find(delim);
                    while (end != std::string::npos)
                    {
                        Vector4 rgba = str_to_eigen(rgba_str.substr(start, end - start));
                        start = end + delim.length();
                        end = rgba_str.find(delim, start);
                        rgbs.push_back(rgba.head(3));
                    }
                    Vector4 rgba = str_to_eigen(rgba_str.substr(start, end));
                    rgbs.push_back(rgba.head(3));
                    body_->set_colors(rgbs);
                } else {
                    Vector4 rgba = str_to_eigen(rgba_str);
